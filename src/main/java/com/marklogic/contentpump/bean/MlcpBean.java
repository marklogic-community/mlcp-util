...
/**
 * Provides a bean-like interface for mlcp. Set whatever properties you need to, then invoke run(). Implements Runnable,
 * just in case that might be helpful for you too.
 */
public class MlcpBean implements Runnable {
    ...
    public void run() {
        try {
            ...
            logArgs(expandedArgs);
            ContentPump.runCommand(expandedArgs);
        } catch (Exception e) {
            throw new RuntimeException("Error running Content Pump: " + e.getMessage(), e);
        }
    }
    ...
    protected void logArgs(String[] args) {
        ...
        logger.info("Invoking Content Pump with args (not showing password): " + argList);
    }
